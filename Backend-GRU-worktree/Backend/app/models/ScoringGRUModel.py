...
class ScoringGRUModel(nn.Module):
    def __init__(
        self,
    ...
    def forward(self, x, lengths=None):
        ...
        # Predict score
        score = self.scorer(context)
        
        return score, temporal_weights, joint_weights

    def predict_frames(self, x):
        """
        Predicts a score for every frame in the sequence.
        x: (batch, num_frames, num_joint_features)
        """
        batch_size, num_frames, num_features = x.shape
        ...
        # Reshape to apply scorer to each frame
        # (batch, num_frames, hidden*2) -> (batch * num_frames, hidden*2)
        scores = self.scorer(gru_out.reshape(batch_size * num_frames, -1))
        
        # Reshape back to (batch, num_frames, num_outputs)
        scores = scores.reshape(batch_size, num_frames, -1)
